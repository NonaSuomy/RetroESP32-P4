...
void Sys_SendKeyEvents(void)
{
    ...
    while (_papp_svc->input_keyboard_read(&event)) {
        /* Quake's console toggle is normally installed by default.cfg.  The
         * PAPP can be launched without that config, so make the physical
         * backtick/tilde key reliable here as well. */
        ...
        if (event.key >= 0 && event.key < 256) {
            /* The launcher supplies a complete press/release pair for text
             * reports. Forward both edges so +forward/+moveleft and other
             * bindings cannot remain latched after a level transition. */
            Key_Event(event.key, event.down != 0);
        }
    }
}
