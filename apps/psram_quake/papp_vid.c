...
{
    (void)rects;
    /* A failed PSRAM allocation must not turn the first video refresh into a
     * store fault.  This is especially important while the launcher is also
     * servicing a diagnostic screen capture. */
    if (s_surface_data[current_surface] == NULL || s_conv_buf == NULL ||
        _papp_svc == NULL || _papp_svc->display_write_frame_custom == NULL)
        return;
