...
{
    /* Doom calls this while its main thread is still unwinding. The mixer
     * task may still be inside music_player->render(), so do not destroy the
     * shared OPL player here. papp_sound_shutdown() stops the task first and
     * performs the actual player teardown below. */
    musicPlaying = false;
}
...
void papp_sound_shutdown(void)
{
    ...
    __builtin_memset(channels, 0, sizeof(channels));
    musicPlaying = false;
}
