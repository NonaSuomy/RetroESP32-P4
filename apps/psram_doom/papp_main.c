...
/* Watchdog: polls the dedicated L3 close control, with MENU as a fallback
   for older launchers that do not expose input_l3_read. */
static void watchdog_task(void *arg)
{
    ...
    for (;;) {
        ...
        const int close_pressed = (_papp_svc->input_l3_read != NULL)
            ? _papp_svc->input_l3_read()
            : state.values[PAPP_INPUT_MENU];
        if (close_pressed) {
            held_ms += 100;
            if (held_ms >= 3000) {
                _papp_svc->log_printf("Doom PAPP: L3 held 3s, requesting exit\n");
                papp_exit_requested = 1;
