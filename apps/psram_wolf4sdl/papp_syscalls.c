/*
 * Newlib syscall stubs + heap wrappers for the Wolf4SDL PSRAM App.
 *
 * Redirects heap and I/O syscalls through app_services_t.
 * Uses --wrap=malloc/free/calloc/realloc at link time.
 *
 * Wolf4SDL opens multiple resource files and config streams simultaneously,
...
extern FILE *__wrap_fopen(const char *path, const char *mode);
extern int __wrap_fclose(FILE *stream);
extern size_t __wrap_fread(void *ptr, size_t size, size_t nmemb, FILE *stream);
extern size_t __wrap_fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream);
extern int __wrap_fseek(FILE *stream, long offset, int whence);
extern long __wrap_ftell(FILE *stream);

/* The SDL 1.2 RWops layer in Wolf4SDL uses these port-specific aliases. */
FILE *__fopen(const char *path, const char *mode) { return __wrap_fopen(path, mode); }
int __fclose(FILE *stream) { return __wrap_fclose(stream); }
size_t __fread(void *ptr, size_t size, size_t nmemb, FILE *stream)
{ return __wrap_fread(ptr, size, nmemb, stream); }
size_t __fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream)
{ return __wrap_fwrite(ptr, size, nmemb, stream); }
int __fseek(FILE *stream, long offset, int whence)
{ return __wrap_fseek(stream, offset, whence); }
long __ftell(FILE *stream) { return __wrap_ftell(stream); }
...
{
    if (_papp_svc) {
        _papp_svc->log_printf("Wolf4SDL cleanup: freeing %d tracked allocations\n", alloc_count);
...
ssize_t __write(int fd, const void *buf, size_t count) { return _write(fd, buf, count); }
...
ssize_t __read(int fd, void *buf, size_t count) { return _read(fd, buf, count); }
...
int __open(const char *path, int flags, ...) { return _open(path, flags, 0); }
...
off_t __lseek(int fd, off_t offset, int whence) { return _lseek(fd, offset, whence); }
...
/* Wolf4SDL calls the ISO C exit() entry point directly from Quit(). The
 * linker wraps it so the app returns to the launcher instead of terminating
 * the process that owns the PAPP task. */
...
/* ── Networking stubs (not used by Wolf4SDL) ──────────────────────────── */
...
void abort(void)
{
    if (_papp_svc) _papp_svc->log_printf("Wolf4SDL PAPP abort() called!\n");
    _exit(1);
}
...
/* Wolf4SDL's Zipit/POSIX compatibility layer uses these underscored names. */
int __unlink(const char *path) { return _unlink(path); }
int __stat(const char *path, struct stat *st) { return _stat(path, st); }
int __mkdir(const char *path, mode_t mode) { (void)path; (void)mode; return 0; }
