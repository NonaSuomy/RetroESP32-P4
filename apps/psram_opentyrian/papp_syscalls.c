...
{
    if (_papp_svc) {
        _papp_svc->log_printf("OpenTyrian cleanup: freeing %d tracked allocations\n", alloc_count);
...
/* heap_caps_malloc — used by SDL shim for PSRAM allocations */
...
void *heap_caps_aligned_alloc(size_t alignment, size_t size, uint32_t caps)
{
    ...
    void *base = _papp_svc->mem_caps_alloc(total, caps);
    if (!base) return NULL;

    uintptr_t raw = (uintptr_t)base + sizeof(void *);
    uintptr_t aligned = (raw + alignment - 1) & ~(uintptr_t)(alignment - 1);
    ((void **)aligned)[-1] = base;
    /* The emulator cores do not free aligned arenas individually; tracking
     * the base keeps cleanup safe when the PAPP returns to the launcher. */
    track_add(base);
    return (void *)aligned;
}
 ...
 * → fseek returns garbage → ftell returns garbage → alloc crash. */
...
/* Some imported cores call ISO C exit() directly.  The PAPP linker wraps it
 * so those cores return through the app's cleanup path instead of stopping
 * the launcher task. */
